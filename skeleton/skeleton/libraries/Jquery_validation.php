...
class Jquery_validation
{
	...
	public function __construct()
	{
		...
		// Load jquery.validate when this one is called.
		if ( ! is_ajax(true) && isset($this->_ci->theme))
		{
			$this->_ci->assets->jquery_validate();
		}
	}
	...
	public function set_rules($rules = array())
	{
		...
		if ( ! empty($rules))
		{
			...
			foreach ($rules as $i => $single)
			{
				...
				{
					if (preg_match("/(.*?)\[(.*)\]/", $rule, $match) && false !== $this->_valid_rule($match[1]))
					{
						$json_rules[$single['field']][$this->_js_rule($match[1])] =  ! empty($this->_ci->config->item($match[2]))
							? $this->_ci->config->item($match[2])
							: (('matches' == $match[1] OR 'differs' == $match[1])
								? '[name='.$match[2].']'
								: $match[2]);
					}
					elseif ( ! preg_match("/callback\_/", $rule) && false !== $this->_valid_rule($rule))
					{
		...
		if ( ! empty($this->_rules))
		{
			...
			foreach ($_rules as $key => $rules)
			{
				...
				foreach ($rules as $rule => $param)
				{
					...
					if (('equalTo' == $rule OR 'notEqualTo' == $rule) && str_contains($param, '[name='))
					{
						$param = rtrim(str_replace('[name=', '', $param), ']');
						$param = $this->_get_field_name($param);
					}
					elseif (is_string($param) && ! empty($this->_ci->config->item($param)))
					{
						$param = $this->_ci->config->item($param);
					}
	...
	private function _output($form = null, $filter = null)
	{
		 ...
		 * @var 	string (function)
		 */
		$jqv_errorPlacement = 'function (error, element) { error.addClass("invalid-feedback"); element.next(".invalid-feedback").remove(); if (element.prop("type") === "checkbox") { error.insertAfter(element.parent("label")); } else { error.insertAfter(element); } }';
		 ...
		 * @since 	2.0
		 */
		if (true !== $this->_ci->uri->is_dashboard)
		{
