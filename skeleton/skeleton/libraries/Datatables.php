 ...
 *
 * Wrapper library based on the native Datatables server-side implementation.
 * @source  http://datatables.net/examples/data_sources/server_side.html
 *
 * @package     CodeIgniter
 * @subpackage  Skeleton
 * @category    Libraries
...
class Datatables
{
	...
	private function get_filtering()
	{
		...
		$sWhere  = '';
		$sSearch = empty($search  = $this->ci->input->post('search')) ? '' :  $this->ci->db->escape_like_str(trim($search['value']));
	...
	private function get_total_results($filtering = false)
	{
		...
		if (isset($this->distinct) && 0 < strlen($this->distinct)) {
	...
	{
		if (empty($column = $this->ci->input->post('columns')))
		{
			return false;
		}

		return is_numeric($column[0]['data']) ? false : true;
	}
