...
final class Route
{
	...
	/**
	 * Cached route names to speed up lookup.
	 * @var arra
	 */
	protected static $cached_routes = array();
	...
	{
		$controller = isset($routes['default_controller']) ? $routes['default_controller'] : self::$default_controller;
	 ...
	 */
	public static function resources($name, $options = array(), $nested = false, $minimal = false)
	{
		...
		if (true === $minimal)
		{
			self::any($name, $new_name.'/index'.$nest_offset, $options, $nested);
			unset($options['as']); // only main GET can be named.

			self::any($name.'/new', $new_name.'/add'.$nest_offset, $options, $nested);
			self::any($name.'/'.$id.'/edit', $new_name.'/edit'.$nest_offset.'/$'.(1 + $offset), $options, $nested);
			self::any($name.'/'.$id, $new_name.'/view'.$nest_offset.'/$'.(1 + $offset), $options, $nested);
		}
		else
		{
	...
	public static function context($name, $controller = null, $options = array())
	{
		...
		if (isset($options['home']) && ! empty($options['home']))
		{
			if (isset($options['as']))
			{
				self::any($name, $options['home'], array('as' => $options['as']));
			}
			else
			{
	 ...
	 *      });
	 */
	public static function prefix($name, Closure $callback, $to = null, $options = array(), $nested = false)
	{
		// Create default route.
		empty($to) OR self::create_route($name, $to, $options, $nested);

		// We first set the prefix.
		// allow nested prefixes.
		self::$prefix = isset(self::$prefix) ? self::$prefix.'/'.$name : $name;
	...
	{
		return empty(self::$prefix) ? '' : implode('/', self::$prefix);
	}
	...
	public static function named($name)
	{
		...
		// Already cached?
		if (isset(self::$cached_routes[$name]))
		{
			return self::$cached_routes[$name];
		}
		...
		self::$cached_routes[$name] = $raw_name.$get;
		return self::$cached_routes[$name];
	}
