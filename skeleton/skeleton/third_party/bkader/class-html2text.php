...
class Html2Text
{
	 ...
	 * @var array $options
	 *
	 * [do_links]   how to treat links.
	 *     none         remove links completely
	 *     inline       shows links inline.
	 *     nextline     shows links on the next line.
	 *     table        if a table of link URLs should be listed after the text.
	 *     bbcode       shows links as bbcode.
	 *
	 * [width]      maximum widt of the formatted text in columns. 0 or less to ignore.
	 */
	...
	public function __construct($html = '', $options = array())
	{
		...
		$this->html = $html;
		$this->options = array_merge($this->options, $options);
		$this->htmlFuncFlags = is_php('5.4') ? ENT_QUOTES | ENT_HTML5 : ENT_QUOTES;
	}
	...
	protected function convertPre(&$text)
	{
		...
		while (preg_match('/<pre[^>]*>(.*)<\/pre>/ismU', $text, $matches))
		{
			...
			$this->preContent = preg_replace_callback($this->callbackSearch, array($this, 'pregCallback'), $this->preContent);

			// convert the content
			$this->preContent = sprintf('<div><br>%s<br></div>', preg_replace($this->preSearch, $this->preReplace, $this->preContent));

			// replace the content (use callback because content can contain $0 variable)
			$text = preg_replace_callback('/<pre[^>]*>.*<\/pre>/ismU', array($this, 'pregPreCallback'), $text, 1);
