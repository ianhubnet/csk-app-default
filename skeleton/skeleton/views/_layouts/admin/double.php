<?php
/**
 * CodeIgniter Skeleton - Administration Double Layout
 *
 * @package     CodeIgniter Skeleton
 * @subpackage  Administration
 */
defined('BASEPATH') OR die;

/**
 * Separated dashboard header.
 * @since   2.12
 */
echo $this->theme->partial('header');

?>

<main id="wrapper" role="main" class="pt-3 pb-5">
	<div class="container">
		<div class="row g-3">
			<div class="col-sm-9">
				<?php
				/**
				 * Display the alert.
				 * @since   2.12
				 */
				echo $this->theme->alert();

				/**
				 * Fires at the top of page content.
				 * @since   1.4
				 */
				do_action('admin_page_header');

				// Display the page content.
				echo $this->theme->content();

				/**
				 * Fires at the end of page content.
				 * @since   1.4
				 */
				do_action('admin_page_footer');

				?>
			</div><!--/.col-->
			<div class="col-sm-3">
				<?php
				/**
				 * Fires at the top of page sidebar.
				 * @since   2.16
				 */
				do_action('admin_sidebar_header');

				/**
				 * Display the admin sidebar.
				 * @since   2.16
				 */
				echo $this->theme->partial('admin-sidebar');

				/**
				 * Fires at the end of page content.
				 * @since   1.4
				 */
				do_action('admin_sidebar_footer');

				?>
			</div><!--/.col-->
		</div><!--/.row-->
	</div><!--/.container-->
</main><!--/#wrapper-->

<?php

/**
 * Separated dashboard footer.
 * @since   2.12
 */
echo $this->theme->partial('footer');
