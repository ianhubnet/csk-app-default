...
class CI_Session
{
	...
	protected function _ci_load_classes($driver)
	{
		...
		{
			require_once(
				is_file(APPPATH.'libraries/Session/Session_driver.php')
					? APPPATH.'libraries/Session/Session_driver.php'
					: BASEPATH.'libraries/Session/Session_driver.php'
			);

			if (is_file($file_path = APPPATH.'libraries/Session/'.$prefix.'Session_driver.php'))
			{
		...
		if ( ! class_exists($class, false) && is_file($file_path = APPPATH.'libraries/Session/drivers/'.$class.'.php'))
		{
			require_once($file_path);
		...
		{
			if (is_file($file_path = APPPATH.'libraries/Session/drivers/'.$class.'.php') OR is_file($file_path = BASEPATH.'libraries/Session/drivers/'.$class.'.php'))
			{
				require_once($file_path);
			}
		...
		if ( ! class_exists($prefix.$class, false) && is_file($file_path = APPPATH.'libraries/Session/drivers/'.$prefix.$class.'.php'))
		{
			require_once($file_path);
	 ...
	 * @return  bool
	 */
	public function mark_as_flash(mixed $key): bool
	{
		if (is_array($key))
		{
	 ...
	 * @return  void
	 */
	public function unmark_flash(mixed $key): void
	{
		if (empty($_SESSION['__ci_vars']))
		{
			return;
	 ...
	 * @return  bool
	 */
	public function mark_as_temp(mixed $key, int $ttl = 300): bool
	{
		$ttl += TIME;
	 ...
	 * @return  void
	 */
	public function unmark_temp(mixed $key): void
	{
		if (empty($_SESSION['__ci_vars']))
		{
			return;
	 ...
	 * @return  void
	 */
	public function set_userdata(mixed $data, mixed $value = null): void
	{
		if (is_array($data))
		{
	 ...
	 * @return  void
	 */
	public function unset_userdata(mixed $key): void
	{
		if (is_array($key))
		{
			foreach ($key as $k)
	 ...
	 * @return  void
	 */
	public function keep_flashdata(mixed $key): void
	{
	 ...
	 * @return  mixed   Session data value or null if not found
	 */
	public function tempdata(string $key = null): mixed
	{
		if (isset($key))
		{
			return (isset($_SESSION['__ci_vars'], $_SESSION['__ci_vars'][$key], $_SESSION[$key]) && is_int($_SESSION['__ci_vars'][$key]))
	 ...
	 * @return  void
	 */
	public function set_tempdata(mixed $data, mixed $value = null, int $ttl = 300): void
	{
		$this->set_userdata($data, $value);
	 ...
	 * @return  void
	 */
	public function unset_tempdata(mixed $key): void
	{
