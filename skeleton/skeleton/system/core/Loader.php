...
class CI_Loader
{
	...
	/**
	 * These arrays are used for '_ci_load_stock_library '
	 *
	 * @var array
	 */
	protected $_ci_core_library_path = array(BASEPATH);
	protected $_ci_app_and_core_library_paths = array(APPPATH, BASEPATH);
	...
	{
		$this->CI = $CI;
		$this->_ci_helper_paths =& $this->_ci_library_paths;
	...
	public function database($params = '', $return = false)
	{
		...
		if ($return === false && isset($this->CI->db) && is_object($this->CI->db) && ! empty($this->CI->db->conn_id))
		{
			return false;
		}
	...
	public function remove_package_path($path = '')
	{
		...
		$this->_ci_library_paths = array_merge_unique($this->_ci_library_paths, array(APPPATH, BASEPATH));
		$this->_ci_model_paths = array_merge_unique($this->_ci_model_paths, array(APPPATH));
		$this->_ci_view_paths = array_merge($this->_ci_view_paths, array(APPPATH.'views/' => true));
		$config->_config_paths = array_merge_unique($config->_config_paths, array(APPPATH));
	...
	protected function _ci_load($_ci_data)
	{
		...
		else
		{
			...
			{
				if (is_file($_ci_view_file.$_ci_file))
				{
		...
		if ( ! $file_exists && ! is_file($_ci_path))
		{
	...
	protected function _ci_load_library($class, $params = null, $object_name = null)
	{
		...
		if (is_file(BASEPATH.'libraries/'.$subdir.$class.'.php'))
		{
	...
	protected function _ci_load_stock_library($library_name, $file_path, $params, $object_name)
	{
		...
		/**
		 * In this situation:
		 * 	1. BASEPATH should be removed.
		 * 	2. APPPATH needs to be the first path checked.
		 */
		$paths = array_unique(array_merge(
			$this->_ci_library_paths,
			array_diff($this->_ci_library_paths, $this->_ci_app_and_core_library_paths)
		));

		foreach ($paths as $path)
		{
			if (is_file($path = $path.'libraries/'.$file_path.$library_name.'.php'))
			{
				// Override
				include_once($path);
				if (class_exists($prefix.$library_name, false))
		...
		foreach ($paths as $path)
		{
			if (is_file($path = $path.'libraries/'.$file_path.$subclass.'.php'))
			{
				include_once($path);
	 ...
	 * @used-by CI_Loader::_ci_load_library()
	 *
	 * @since 	2.99 	Tweaked to make app config useable.
	 *
	...
	protected function _ci_init_library($class, $prefix, $config = false, $object_name = null)
	{
		...
		if ($config === null)
		{
			...
			if (is_array($config_component->_config_paths))
			{
				...
				{
					// If we are inspecting Skeleton config, we do not break
					// so that we can load application's config.
					if ($path === KBPATH)
					{
					...
					elseif (is_file($filepath = $path.'config/'.strtolower($class).'.php'))
					{
						include_once($filepath);
						$found = true;
					}
					elseif (is_file($filepath = $path.'config/'.ucfirst(strtolower($class)).'.php'))
					{
						include_once($filepath);
						$found = true;
					}

					if (is_file($path.'config/'.ENVIRONMENT.'/'.strtolower($class).'.php'))
					{
						include_once($filepath);
					}
					elseif (is_file($filepath = $path.'config/'.ENVIRONMENT.'/'.ucfirst(strtolower($class)).'.php'))
					{
						include_once($filepath);
						$found = true;
					}
		...
		{
			if ($this->CI->$object_name instanceof $class_name)
			{
		...
		$this->CI->$object_name = isset($config)
