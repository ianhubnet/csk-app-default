...
class CI_Log
{
	...
	public function __construct()
	{
		...
		$this->_log_path = normalize_path(($config['log_path'] !== '') ? $config['log_path'] : APPPATH.'logs/');
		$config['log_path'] = $this->_log_path; // force set config

		$this->_log_filename = (isset($config['log_filename']) && $config['log_filename'] !== '')
			? $config['log_filename'] : 'log-'.date('Y-m-d').'.php';

		is_dir($this->_log_path) OR mkdir($this->_log_path, 0755, true);
