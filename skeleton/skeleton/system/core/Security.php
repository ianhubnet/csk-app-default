...
class CI_Security
{
	 ...
	 * @return  void
	 */
	public function __construct(CI_Config &$config, CI_Router &$router, string $charset = 'UTF-8')
	{
		$this->config = $config;
		$this->router = $router;
		...
		if ($this->config->item('csrf_protection') && ! is_cli())
		{
			// CSRF config
			foreach (array('csrf_expire', 'csrf_token_name', 'csrf_cookie_name') as $key)
			{
				if (null !== ($val = $this->config->item($key)))
				{
					$this->{'_'.$key} = $val;
				}
			}
			...
			if ($cookie_prefix = $this->config->item('cookie_prefix'))
			{
	...
	public function csrf_verify()
	{
		...
		if ($exclude_uris = $this->config->item('csrf_exclude_uris'))
		{
	...
	public function csrf_set_cookie()
	{
		...
		$secure_cookie = (bool) $this->config->item('cookie_secure');
		...
		{
			$domain = trim($this->config->item('cookie_domain'));
			header('Set-Cookie: '.$this->_csrf_cookie_name.'='.$this->_csrf_hash
					.'; Expires='.gmdate('D, d-M-Y H:i:s T', $expire)
					.'; Max-Age='.$this->_csrf_expire
					.'; Path='.implode('/', array_map('rawurlencode', explode('/', $this->config->item('cookie_path'))))
					.($domain === '' ? '' : '; Domain='.$domain)
					.($secure_cookie ? '; Secure' : '')
					.($this->config->item('cookie_httponly') ? '; HttpOnly' : '')
					.'; SameSite=Strict'
			);
		}
