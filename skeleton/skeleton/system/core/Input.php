...
class CI_Input
{
	...
	public function set_cookie($name, $value = '', $expire = 0, $domain = '', $path = '/', $prefix = '', $secure = null, $httponly = null, $samesite = null)
	{
		...
		if ($prefix === '' && $this->config->item('cookie_prefix', null, '') !== '')
		{
			$prefix = $this->config->item('cookie_prefix');
		}

		if ($domain == '' && $this->config->item('cookie_domain', null, '') != '')
		{
			$domain = $this->config->item('cookie_domain');
		}

		if ($path === '/' && $this->config->item('cookie_path', null, '/') !== '/')
		{
			$path = $this->config->item('cookie_path');
		}

		$secure = ($secure === null && $this->config->item('cookie_secure') !== null)
			? (bool) $this->config->item('cookie_secure')
			: (bool) $secure;

		$httponly = ($httponly === null && $this->config->item('cookie_httponly') !== null)
			? (bool) $this->config->item('cookie_httponly')
			: (bool) $httponly;
		...
		isset($samesite) OR $samesite = $this->config->item('cookie_samesite');
		if (isset($samesite))
		{
	...
	public function ip_address()
	{
		...
		elseif ( ! empty($proxy_ips = $this->config->item('proxy_ips')) && ! is_array($proxy_ips))
		{
